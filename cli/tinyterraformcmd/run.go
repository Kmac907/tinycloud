...
func RuntimeWrapperEnv(cwd, terraformDir string, lookPath func(string) (string, error)) ([]string, func(), error) {
	...
	azShimDir, azShimLog, err := EnsureTinyTerraformAzShim(runtimeRoot, repoRoot, lookPath)
	if err != nil {
		if runtimeCleanup != nil {
			runtimeCleanup()
		}
...
func EnsureTinyTerraformAzShim(runtimeRoot, repoRoot string, lookPath func(string) (string, error)) (string, string, error) {
	...
	shimScript, err := LoadTinyTerraformAzShimScript(repoRoot)
	if err != nil {
		return "", "", err
	}

	shimScriptPath := filepath.Join(shimDir, "azshim.ps1")
	if err := os.WriteFile(shimScriptPath, []byte(shimScript), 0o644); err != nil {
