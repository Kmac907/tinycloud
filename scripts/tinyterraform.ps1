...
function Resolve-TinyCloudSourceRoot {
    ...
    $topLevelMain = Join-Path $repoRoot "cmd\tinycloud\main.go"
    if (Test-Path $topLevelMain) {
        return $repoRoot
    }

    $azureSourceRoot = Join-Path $repoRoot "azure"
    $azureMain = Join-Path $azureSourceRoot "cmd\tinycloud\main.go"
    if (Test-Path $azureMain) {
        return $azureSourceRoot
    }
    ...
    $discovered = Find-UpwardPath -Start $PSScriptRoot -RelativePath "azure\cmd\tinycloud\main.go"
    if (-not [string]::IsNullOrWhiteSpace($discovered)) {
        return (Join-Path $discovered "azure")
    }
...
$tinycloudSourceRoot = Resolve-TinyCloudSourceRoot
...
if ($terraformSubcommand -eq "init") {
    $tinyterraformRepoRoot = Resolve-TinyTerraformRepoRoot -TinyCloudSourceRoot $tinycloudSourceRoot
...
$server = $null
try {
    $server = Start-Process -FilePath $tinycloudExe -ArgumentList "start" -PassThru -RedirectStandardOutput $serverStdout -RedirectStandardError $serverStderr -WorkingDirectory $tinycloudSourceRoot
